...
class Queries {
  ...
  const NEW_ADMIN_BY_UID = 'INSERT INTO OperatorEntitlement(OperatorId, EntitlementId) VALUES ((SELECT UserId FROM Operator WHERE Operator.OperatorId = ?), 3)';
  const UPDATE_OPERATOR_BY_UID = 'UPDATE Operator SET Title=?, HIGHESTDEGREE=? WHERE UserId = ?';
  const UPDATE_OPERATOR_BY_OPID = 'UPDATE Operator SET Title=?, HIGHESTDEGREE=? WHERE OperatorId = ?';
